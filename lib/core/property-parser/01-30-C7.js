...
* - EPC              : C7 : Air purifier function setting
* ---------------------------------------------------------------- */
'use strict';
...
var EchonetLitePropertyParser = function() {
	...
	this.descs = {
		'en': {
			'DST': {
				'field': 'Electrical dust collection-based air purifier function',
				'values': {}
			},
			'ION': {
				'field': 'Cluster ion-based air purifier function',
				'values': {}
			},
			'LVL': {
				'field': 'Level',
...
EchonetLitePropertyParser.prototype.parse = function(buf) {
	...
	var offset = 0;
	var message = {};
	['DST', 'ION'].forEach((bse_key) => {
		var msg_key = (bse_key === 'DST') ? 'dust' : 'ion';
		...
		var lvl_buf = new Buffer([lvl_value]);
		bse['structure'].push({
			'key'      : lvl_key,
		...
		var mod_buf = new Buffer([mod_value]);
		bse['structure'].push({
			'key'      : mod_key,
		...
		var stt_buf = new Buffer([stt_value]);
		bse['structure'].push({
			'key'      : stt_key,
...
EchonetLitePropertyParser.prototype.create = function(data) {
	...
	buf_list.push(new Buffer([0x00, 0x00, 0x00, 0x00, 0x00, 0x00]))

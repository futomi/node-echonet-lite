...
* - EPC              : D5 : Instance list notification
* ---------------------------------------------------------------- */
'use strict';
...
var EchonetLitePropertyParser = function() {
	...
	this.descs = {
		'en': {
			'NUM': 'Number of notification instances'
		},
		'ja': {
			'NUM': '通報インスタンス数'
		}
...
EchonetLitePropertyParser.prototype.parse = function(buf) {
	...
	// Parse each EOJ
	var instans_list = [];
	for(var offset=1; offset<buf.length; offset+=3) {
		var instance = {};
		...
		// EOJ X3 (Instance code)
		var eojx3_key = 'EOJX3';
		var eojx3_value = buf.readUInt8(offset + 2);
		var eojx3_buf = buf.slice(offset + 2, offset + 3);
		var eojx3 = {
			'key'   : eojx3_key,
			'field' : mELDesc.getFieldName(eojx3_key),
			'value' : eojx3_value,
			'buffer': eojx3_buf,
			'hex'   : mBuffer.convBufferToHexString(eojx3_buf),
			'desc'  : eojx3_value.toString()
		};
		eoj['structure'].push(eojx3);
		...
		instans_list.push({
			...
			'instance'  : eojx3_value
		});
		*/
		instans_list.push([eojx1_value, eojx2_value, eojx3_value]);
	}
	var parsed = {
		'message': {
			'list': instans_list
...
EchonetLitePropertyParser.prototype.create = function(data) {
	...
	list.forEach((eoj) => {
		...
		var c = eoj[1];
		var i = eoj[2];
		if(typeof(g) !== 'number' || typeof(c) !== 'number' || typeof(i) !== 'number') {
			throw new Error('The "list" property in the 1st argument "data" is invalid.');
		}
		if(g < 0 || g > 0xFF || c < 0 || c > 0xFF || i < 0 || i > 0xFFF) {
			throw new Error('The "list" property in the 1st argument "data" is invalid.');
		}
		byte_list.push(g);
		byte_list.push(c);

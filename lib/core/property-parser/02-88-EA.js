...
* - EPC              : EA : Cumulative amounts of electric energy measured at fixed time (normal direction)
* ---------------------------------------------------------------- */
'use strict';
...
EchonetLitePropertyParser.prototype.parse = function(buf) {
	...
	var hms_key = 'HMS';
	var hms_value = Buffer.concat([Buffer.from([0x00]), hms_buf]).readUInt32BE(0);
...
EchonetLitePropertyParser.prototype.create = function(data) {
	...
	var m_buf = Buffer.alloc(1);
	m_buf.writeUInt8(m);
	var s_buf = Buffer.alloc(1);
	s_buf.writeUInt8(s);
	var e_buf = Buffer.alloc(4);

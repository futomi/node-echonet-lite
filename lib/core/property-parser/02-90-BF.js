...
* - EPC              : BF : Lighting mode status in auto mode
* ---------------------------------------------------------------- */
'use strict';
...
var EchonetLitePropertyParser = function() {
	...
	this.descs = {
		'en': {
			'MOD': {
				'field': 'Lighting mode status in auto mode',
				'values': {
					0x42: 'Main lighting',
					0x43: 'Night lighting',
					0x44: 'Off',
...
EchonetLitePropertyParser.prototype.create = function(data) {
	...
	} else if(!(mode === 0x42 || mode === 0x43 || mode === 0x44 || mode === 0x45)) {
		throw new Error('The `mode` must be 0x40, 0x41, 0x42, 0x43, or 0x44.');
	}

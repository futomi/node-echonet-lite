...
EchonetLiteCore.prototype.create = function(data) {
	...
	var prop = null;
	if(data['prop']) {
		prop = JSON.parse(JSON.stringify(data['prop']));
	}
	...
	var ehd1_buf = new Buffer([0b00010000]);
	var ehd2_buf = new Buffer([0x81]);
	var tid_buf = new Buffer(2);
	tid_buf.writeUInt16BE(tid);
	var seoj_buf = new Buffer(seoj);
	var deoj_buf = new Buffer(deoj);
	var esv_buf = new Buffer([esv]);
	var opc = prop.length;
	var opc_buf = new Buffer([opc]);
	...
	prop.forEach((o) => {
		...
		if(edt) {
			...
			var pdc_buf = new Buffer([edt_buf.length]);
			buf_list.push(epc_buf, pdc_buf, edt_buf);
		} else {
			var pdc_buf = new Buffer([0x00]);

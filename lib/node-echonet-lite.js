...
*              Optional, the default value is 'en'.
*     type   : Network type code, 'lan' or 'wisunb'.
*              Required.
*     adapter: Product name of Wi-SUN USB dongle, 'bp35a1', or 'rl7023'.
*              Required if the type is 'wisunb'.
*     path   : Path of the serial port (e.g.'COM3').
*              Required if the type is 'wisunb'.
*     id     : Route-B ID.
*              Required if the type is 'wisunb'.
*     pass   : Route-B Password.
*              Required if the type is 'wisunb'.
*     baud   : Baud rate of the serial port.
*              Optional, the default vlaue is 115200.
* ---------------------------------------------------------------- */
var EchonetLite = function(params) {
	...
	if(this.type === 'wisunb') {
		...
		if(('adapter' in params) && typeof(params['adapter'] === 'string') && params['adapter'] !== '') {
			...
			if(adapter.match(/^(bp35a1|rl7023)$/i)) {
...
EchonetLite.prototype._prepareDiscovery = function(callback) {
	var tid = this._addCallback((err, parsed) => {
		...
		var is_my_packet_echo = (
			seoj[0] === this.discovery_seoj[0] &&
			seoj[1] === this.discovery_seoj[1] &&
			seoj[2] === this.discovery_seoj[2] &&
